...
static char rcsid[] = "$Id: gen.c,v 1.1.1.1 2002/06/25 08:47:11 jmaginni Exp $";
...
int range(Node p, int lo, int hi) {
	...
	case CALL+I:  ck(s->u.c.v.i  >= lo && s->u.c.v.i  <= hi);
	case CALL+U:  ck(s->u.c.v.i  >= lo && s->u.c.v.i  <= hi);
	case CALL+V:  ck(s->u.c.v.i  >= lo && s->u.c.v.i  <= hi);
	}
	return LBURG_MAX;
}
...
static void putreg(Symbol r) {
	...
	freemask[r->x.regnode->set] |= 0xffffffff;/*r->x.regnode->mask; */
