...
static char rcsid[] = "$Id: dag.c,v 1.1.1.1 2002/06/25 08:46:46 jmaginni Exp $";
...
static Node visit(Node, int, Node);
...
static void trybreak(Node forest) {
	Node p;
...
static void dagused(Node p, Node q, int *counter) {
	if (!p)
		return;
	if (p==q) {
		(*counter)--;
	...
	dagused(p->kids[0],q,counter);
	dagused(p->kids[1],q,counter);	
}

static int breaks(Node p, Node state) {
	int ucount;
	...
	dagused(state->kids[0],p,&ucount);
	dagused(state->kids[1],p,&ucount);
	if (ucount>0)
		return 1;
	else
		return 0;	
...
static Node visit(Node p, int listed, Node state) {
	if (p) 
		if (p->syms[2])
			p = tmpnode(p);
		else if (p->count <= 1 && !iscall(p->op)
		||	p->count == 0 && iscall(p->op)) {
			p->kids[0] = visit(p->kids[0], 0, state);
			p->kids[1] = visit(p->kids[1], 0, state);
		}
/*		else if (p->count == 1 && iscall(p->op) 
			&& generic(state->op)!= ARG 
			&& state->link 
			&& generic(state->link->op)!=ARG ) {
				fprintf(stderr,"ARSE!\n");
				printdag(state,0); 
				p->kids[0] = visit(p->kids[0], 0, state);
				p->kids[1] = visit(p->kids[1], 0, state);
		...
		else if (!iscall(p->op) && !breaks(p, state)) {
/*			fprintf(stderr,"STINGING LOCAL\n"); 
			printdag(p,0); */
			p->count--;
...
/*			fprintf(stderr,"This should be a register\n");
			printdag(p,0);*/
			p->kids[0] = visit(p->kids[0], 0, state);
			p->kids[1] = visit(p->kids[1], 0, state);
